...
//   }
  ...
  public async groupChat(
    ...
    const t = await sequelize.transaction();
    try {
      const { name, description, participant } = req.body;

      if (!name || !this.currentUserId) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid request payload" });
      }

      const response = await this.chatService.groupChat(name, description, participant);
